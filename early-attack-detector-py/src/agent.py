...
def detect_malicious_contract_tx(
    ...
    for trace in transaction_event.traces:
        if trace.type == "create":
            nonce_address = w3.eth.getTransactionCount(w3.toChecksumAddress(transaction_event.from_), transaction_event.block_number)
...
def process_finding(iter_finding, address_from, created_contract_address, tx_timestamp):
    ...
    # Let's only process if it's over info threshold AND has a label.
    if (len(info_labels) > 0 and float(finding.metadata['model_score']) >= MODEL_THRESHOLD):
