...
from constants import THORCHAIN_ROUTER_ADDRESS
...
class TestThorchainFundingBot:
    ...
    async def test_transfer_to_contract(self, mocker):
        ...
        findings = await agent.detect_thorchain_funding(w3, tx_event)
        assert len(findings) == 0, "This should have not triggered a finding as the to is a contract"


    @pytest.mark.asyncio
    @patch('agent.get_chain_id', return_value=1)
    async def test_not_transfer_from_thorchain(self, mocker):
        agent.initialize()
    ...
    @pytest.mark.asyncio
    @patch('agent.get_chain_id', return_value=1)
    async def test_transfer_from_thorchain_to_new_account(self, mocker):
        agent.initialize()
    ...
    @pytest.mark.asyncio
    @patch('agent.get_chain_id', return_value=1)
    async def test_low_value_transfer_from_thorchain(self, mocker):
        agent.initialize()
    ...
    @pytest.mark.asyncio
    @patch('agent.get_chain_id', return_value=1)
    async def test_high_value_transfer_from_thorchain(self, mocker):
        agent.initialize()
