...
@functools.lru_cache(maxsize=128)
def _get_event_data(logs: tuple, abi: str, codec: ABICodec) -> list:
    _abi = ABIEvent(json.loads(abi))
    # parse logs
    _results = []
    for _log in logs:
...
@functools.lru_cache(maxsize=128)
def parse_logs(logs: tuple, abi: str=ERC20_TRANSFER_EVENT, codec: ABICodec=_abi_codec()) -> tuple:
    """Extract all the event matching a given ABI."""
    _inputs = _get_input_names(abi)
    _events = _get_event_data(logs=logs, abi=abi, codec=codec)
