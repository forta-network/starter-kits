...
class TestMaliciousSmartContractML:
            ...
            traces = [
                {
                    ...
                    "action": {
                        "from": MALICIOUS_CONTRACT_DEPLOYER,
                        "init": await w3.eth.get_code(MALICIOUS_CONTRACT),
                        "value": 1,
                    },
    ...
    @pytest.mark.asyncio
    @patch('agent.get_chain_id', return_value=1)
    async def test_detect_malicious_contract_benign(self, mocker):
        await agent.initialize()
        bytecode = await w3.eth.get_code(BENIGN_CONTRACT)
        findings = await agent.detect_malicious_contract(
    ...
    @pytest.mark.asyncio
    async def test_detect_malicious_contract_short(self):
        await agent.initialize()
