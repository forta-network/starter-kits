...
class TestScamFindings:
    def test_scam_finding_model_strict(self):
        ...
        finding = ScamDetectorFinding.scam_finding_model(block_chain_indexer, EOA_ADDRESS_2, 0.9, "SCAM-DETECTOR-MODEL-1", feature_vector, alerts, 1)  # this EOA did not deploy a contract
        ...
        assert finding.metadata['feature_vector'] == feature_vector_str, "should be the same"
    ...
    def test_scam_finding_model_loose(self):
        ...
        finding = ScamDetectorFinding.scam_finding_model(block_chain_indexer, EOA_ADDRESS_2, 0.6, "SCAM-DETECTOR-MODEL-2", feature_vector, alerts, 1)  # this EOA did not deploy a contract
        assert finding.alert_id == "SCAM-DETECTOR-MODEL-2", "should be SCAM-DETECTOR-MODEL-2"
        assert finding.description == f'{EOA_ADDRESS_2} likely involved in an attack (SCAM-DETECTOR-MODEL-2)', "should be SCAM-DETECTOR-MODEL-2"
        assert len(finding.labels) == 1, "should be 1"
